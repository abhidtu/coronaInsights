...
public class CasesDaoImpl extends CasesDao {
    ...
    public List<CoronaVirusCountryWiseData> aggregateDataForCountry(String country) {
        ...
        List<CoronaVirusCountryWiseData> coronaVirusCountryWiseData = DSL.using(configuration())
                                         .select(aggregated.field("aggregated_reporting_date").as("reportedDate"), DSL.sum(aggregated.field("aggregated_confirmed").coerce(Integer.class)).as("confirmed"), DSL.sum(aggregated.field("aggregated_deaths").coerce(Integer.class)).as("deaths"), DSL.sum(aggregated.field("aggregated_recovered").coerce(Integer.class)).as("recovered")).from(aggregated).groupBy(aggregated.field("aggregated_reporting_date")).orderBy(aggregated.field("aggregated_reporting_date").desc()).fetchInto(CoronaVirusCountryWiseData.class);
        return coronaVirusCountryWiseData;
    }
